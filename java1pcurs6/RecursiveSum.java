...
public class RecursiveSum extends RecursiveTask<Long> {
    ...
    public Long compute() {
        if(to - from < 1000) {
            //suma a 1000 de numere poate fi calculata 
            //pe un singur fir de executie
            
            long sum = 0;
            for(int i = from; i <= to; i++) {
                sum += i;
            }
            System.out.println("Suma intre " + from + " si " + to +
                               " este: " + sum);
            return sum;
        } else {
            //trebuie sa impartim intervalul de numere
            //in intervale mai mici, care sa fie prelucrate
            //in paralel pe mai multe fire de executie
            int middle = (from + to) / 2;
            
            System.out.println("Am separat in suma intre " +
                                 from + " - " + middle +
                                 " si suma intre " +
                                 (middle + 1) + " - " + to);
